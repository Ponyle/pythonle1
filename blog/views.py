...
def index(request):  # 首页
    ...
    article_list1 = Article.objects.filter(title__startswith=msg).order_by('create_time').reverse()
    testdir=r"E:\Program Files\Zblog\static\upload"
    ls = os.listdir(testdir)
    # 对文件修改时间进行升序排列
    dir_list = sorted(ls, key=lambda x: os.path.getmtime(os.path.join(testdir, x)))
    tx = dir_list[-1]
    article_list = Article.objects.order_by('create_time').reverse()# 查询所有文章
    for m in range(len(article_list)):
        if article_list[m].title=='':
            article_list[m].title='暂无标题'



    num1 = len(article_list)-1
    list1 = []
    ls1 = Article.objects.order_by('comment_count').reverse()
    for i in ls1:
    ...
    paginator = Paginator(article_list, 4)  # 每页显示4条

    list2 = []
    ls2 = Article.objects.order_by('up_count').reverse()
    for j in ls2:
        if j.up_count==0:
            pass
        else:
    ...
    return render(request, "index.html", {"msg1":msg1,"result2":result2,"msg":msg,"result":result,"num1":num1,"num":len(article_list1),"article_list1":article_list1,"article_list": article_list,"paginator": paginator, "current_num": current_num,"pageRange":pageRange,"tx":tx})
...
def homesite(request, username, **kwargs):  # 个人站点主页
    ...
    # 查询当前用户发布的所有文章
    if not kwargs:
        article_list = Article.objects.filter(user__username=username).order_by('create_time').reverse()
    else:
        ...
        if condition == "category":
            # 查询当前分类的文章列表
            article_list = Article.objects.filter(user__username=username).filter(category__title=params).order_by('create_time').reverse()
        elif condition == "tag":
            # 查询当前标签的文章列表
            article_list = Article.objects.filter(user__username=username).filter(tags__title=params).order_by('create_time').reverse()
        else:
            # 查询当前归档的文章列表
            year, month = params.split("/")
            article_list = Article.objects.filter(user__username=username).filter(create_time__year=year,
                                                                                  create_time__month=month).order_by('create_time').reverse()
...
def article_detail(request, username, article_id):  # 文章详情
    ...
    comment_list = Comment.objects.filter(article_id=article_id).order_by('create_time').reverse()
    dict = {"blog": blog,
            "username": username,
...
def backend(request):  # 后台管理首页
    user = request.user
    # 当前用户文章列表
    article_list = Article.objects.filter(user=user).order_by('create_time').reverse()
...
def upload(request):  #上传文件
    ...
    name = obj.name  # 文件名
    # 文件存储的绝对路径
    path = os.path.join(settings.BASE_DIR, "static", "11", name)
...
def manage_comment(request):  # 管理评论
    # 当前用户所有文章的评论
    comment_list = Comment.objects.filter(article__user_id=request.user.pk).order_by('create_time').reverse()
...
def delete_comment(request):  # 删除评论
    ...
    if request.method == "POST":
        id = request.POST.get("id")
        comment_list = Comment.objects.filter(article__user_id=request.user.pk).order_by('create_time').reverse()
        ls = Comment.objects.filter(nid=id).all()
        for i in ls:
            a = i.article.nid
            print(i.parent_comment_id)
        ...
        with transaction.atomic():  # 增加事务
            ret = Comment.objects.filter(pk=id).delete()  # 返回元组
            # msg = Article.objects.filter(nid=a).all()
            Article.objects.filter(pk=a).update(comment_count=F("comment_count") - 1)
        if ret[0]:  # 取值为1的情况下
            response['state'] = True
        else:  # 取值为0的情况下
            response['state'] = False
